...
class NotarPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('notar.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `notar`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM notar WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO notar (date, title, link, content)"
